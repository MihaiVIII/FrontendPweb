import { UserRegisterFormController, UserRegisterFormModel } from "./UserRegisterForm.types";
 ...
 */
const getDefaultValues = (initialData?: UserRegisterFormModel) => {
    const defaultValues = {
        email: "",
        name: "",
        password: "",
...
const useInitUserAddForm = () => {
    ...
    const schema = yup.object().shape({
        ...
        password2: yup.string()
                .oneOf([yup.ref("password")],formatMessage({id:"globals.validations.passmatch"}))
                .required(formatMessage(
                    { id: "globals.validations.requiredField" },
                    {
                        fieldName: formatMessage({
 ...
 */
export const useUserRegisterFormController = (onSubmit?: () => void): UserRegisterFormController => {
    ...
    const submit = useCallback((data: UserRegisterFormModel) => // Create a submit callback to send the form data to the backend.
        add(data).then(() => {
            ...
            redirectToLogin()
        }), [add, queryClient,redirectToLogin, queryKey]);
    ...
    const {
        ...
        setValue,
        formState: { errors }
    } = useForm<UserRegisterFormModel>({ // Use the useForm hook to get callbacks and variables to work with the form.
