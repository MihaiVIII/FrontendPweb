import { ItemAddFormController, ItemAddFormModel } from "./ItemAddForm.types";
 ...
 */
const getDefaultValues = (initialData?: ItemAddFormModel) => {
    const defaultValues = {
        name: "",
        price: 0,
        quantity: 0,
 ...
 */
export const useItemAddFormController = (onSubmit?: () => void): ItemAddFormController => {
    const { defaultValues, resolver } = useInitItemAddForm();
    const { AdminaddItem: { mutation, key: mutationKey }, getItems: { key: queryKey } } = useProdApi();
    ...
    const submit = useCallback((data: ItemAddFormModel) => // Create a submit callback to send the form data to the backend.
        add(data).then(() => {
    ...
    const {
        ...
        setValue,
        formState: { errors }
    } = useForm<ItemAddFormModel>({ // Use the useForm hook to get callbacks and variables to work with the form.
    ...
    const selectRole = useCallback((event: SelectChangeEvent<String>) => { // Select inputs are tricky and may need their on callbacks to set the values.
        setValue("userId", event.target.value as string, {
            shouldValidate: true,
        });
    }, [setValue]);
