...
import { useProdTableController } from "./ProductsTable.controller";
import { ItemDTO } from "@infrastructure/apis/client";
import DeleteIcon from '@mui/icons-material/Delete';
import { OwnItemAddDialog } from "../../Dialogs/ItemAddDialog";
...
export const ProductsTable = () => {
    ...
    const { handleChangePage, handleChangePageSize, pagedData, isError, isLoading, tryReload, labelDisplay, remove } = useProdTableController(); // Use the controller hook.
    
    const rowValues = getRowValues(pagedData?.data, orderMap); // Get the row values.
    
    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        <OwnItemAddDialog /> {/* Add the button to open the user add modal. */}
